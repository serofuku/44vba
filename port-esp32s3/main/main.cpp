...
static uint16_t *FB_front = NULL;  // being sent to display
static uint16_t *FB_back  = NULL;  // being written by emulator

// Sync primitives
static SemaphoreHandle_t frameSemaphore  = NULL;  // signals frame ready
static SemaphoreHandle_t bufferMutex     = NULL;  // protects buffer swap
...
static void displayTask(void *arg) {
    while (1) {
        // Wait for emulator to signal a frame is ready
...
extern "C" void app_main() {
    // Lock CPU to 240MHz on both cores
    ...
    // Launch display task on Core 0 (priority 5)
    xTaskCreatePinnedToCore(
        displayTask, "disp",
        4096, NULL, 5, NULL, 0);

    // Launch emulator task on Core 1 (priority 5)
    xTaskCreatePinnedToCore(
        emuTask, "emu",
        8192, NULL, 5, NULL, 1);

    // app_main exits here — tasks keep running
}
